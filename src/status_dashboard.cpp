...
static void status_render(const Config &config, pid_t pid, double uptime, int incident_count)
{
    ...
    std::cout << "  [last]     " << timebuf << " (" << static_cast<long>(diff) << "s ago)\n";
    if (mod_time == 0)
    ...
    std::cout << "  [webhook]  " << (config.webhook_url.empty() ? "not configured" : config.webhook_url) << "\n";
