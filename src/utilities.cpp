...
std::string write_incident(const Config &config, const std::string &keyword,
  ...
  if (ec) {
    std::cerr << "[incident] error: cannot create directory "
              << config.incident_dir << ": " << ec.message() << "\n";
    return "";
  }

  std::string filename =
      config.incident_dir + "/incident_" + get_timestamp() + ".log";
...
void print_config_summary(const Config &config, const std::string &prefix) {
  ...
  std::cout << prefix << "cooldown:    "
            << (config.cooldown_seconds > 0
                    ? std::to_string(config.cooldown_seconds) + "s"
                    : "none")
            << "\n";
}
