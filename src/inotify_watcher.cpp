...
InotifyWatcher::InotifyWatcher(const std::string &filepath)
    : filepath_(filepath), running_(true), inotify_fd_(-1), file_watch_fd_(-1),
      dir_watch_fd_(-1), file_fd_(-1), read_offset_(0), current_inode_(0) {
  wake_pipe_[0] = -1;
  wake_pipe_[1] = -1;
...
void InotifyWatcher::setup_watches() {
  ...
  dir_watch_fd_ =
      inotify_add_watch(inotify_fd_, dir_path_.c_str(),
                        IN_CREATE | IN_MOVED_TO | IN_MOVED_FROM | IN_DELETE);
  if (dir_watch_fd_ < 0) {
    dir_watch_fd_ = -1;
    std::cerr << "[swl] warning: cannot watch directory for rotation (dir: "
              << dir_path_ << ")\n";
  }
}
    ...
    while (ptr < event_buf + len) {
      ...
      if (event->wd == file_watch_fd_ && (event->mask & IN_MODIFY)) {
        if (file_fd_ >= 0) {
          struct stat st;
          if (stat(filepath_.c_str(), &st) == 0 &&
              st.st_ino == current_inode_) {
            if (st.st_size >= 0 &&
                static_cast<off_t>(st.st_size) < read_offset_)
              read_offset_ = 0;
          }
        }
      }
