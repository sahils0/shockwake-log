...
WebhookAlert::WebhookAlert(const std::string &webhook_url, int max_retries,
                           int retry_delay_ms, bool ssl_verify)
    : webhook_url_(webhook_url), max_retries_(max_retries),
      retry_delay_ms_(retry_delay_ms), ssl_verify_(ssl_verify), running_(false),
      worker_thread_(0), dropped_count_(0) {
...
void WebhookAlert::worker_loop() {
  while (running_) {
    PendingAlert alert;
    {
      ...
      queue_cv_.wait(lock,
                     [this]() { return !alert_queue_.empty() || !running_; });
    ...
    if (send_post(alert.payload)) {
      ...
      std::cerr << "[webhook] Delivery failed (attempt " << alert.attempt + 1
                << "/" << max_retries_ << "), retrying in " << retry_delay_ms_
                << "ms...\n";
