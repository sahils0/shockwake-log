...
static void parse_triggers(Config &config, const std::string &value) {
  ...
  while ((pos = remaining.find(',')) != std::string::npos) {
    ...
    if (!token.empty())
      config.triggers.push_back(token);
    remaining.erase(0, pos + 1);
  }
  std::string last = trim(remaining);
  if (!last.empty())
    config.triggers.push_back(last);
}
  ...
  while (std::getline(file, line)) {
    ...
    if (key == "log") {
      ...
      if (!validate_webhook_url(value))
        std::cerr
            << "warning: webhook url should start with http:// or https://\n";
...
static std::string discover_log_file() {
  ...
  std::vector<std::string> local_patterns = {"./*.log", "./logs/*.log",
                                             "./log/*.log"};
    ...
    for (const auto &entry : fs::directory_iterator(dir)) {
      ...
      if (name.size() > 4 && name.substr(name.size() - 4) == ".log") {
        if (fs::exists(entry.path()) &&
            (static_cast<int>(fs::status(entry.path()).permissions()) &
             static_cast<int>(fs::perms::owner_read))) {
  ...
  std::vector<std::string> system_paths = {
      ...
      "/var/log/auth.log", "/var/log/nginx/error.log", "/var/log/docker.log",
  };

  for (const auto &path : system_paths) {
    if (fs::exists(path) && (static_cast<int>(fs::status(path).permissions()) &
                             static_cast<int>(fs::perms::owner_read))) {
      return path;
    }
  }
...
void print_usage(const char *program) {
  ...
  std::cerr
      << "swl — Real-time log anomaly detection with contextual debugging.\n\n"
      << "Usage:\n"
      << "  " << name
      << "                         Auto-discover and monitor logs (sensible "
         "defaults)\n"
      << "  " << name
      << " <logfile>               Monitor a specific log file\n"
      << "  " << name
      << " [options]               Monitor with custom options\n\n"
      << "Subcommands:\n"
      << "  " << name
      << " init                  Generate config file in current directory\n"
      << "  " << name
      ...
      << "Options:\n"
      << "  --log <path>          Log file to monitor (auto-discovered if "
         "omitted)\n"
      << "  --webhook <url>       Webhook URL for alerts (omit for local-only "
         "logging)\n"
      << "  --config <file>       Path to config file (auto-loaded from "
         "standard locations)\n"
      << "  --triggers <k1,k2>    Comma-separated keywords or regex (default: "
         "FATAL,ERROR)\n"
      << "  --exclude <e1,e2>     Exclusion patterns — lines matching these "
         "are skipped\n"
      << "  --window <n>          Lines to keep in context buffer (default: "
         "100)\n"
      << "  --trailing <n>        Lines to capture after trigger (default: "
         "20)\n"
      << "  --incidents <dir>     Directory for incident reports (default: "
         "/var/log/swl-incidents)\n"
      << "  --cooldown <sec>      Min seconds between alerts per trigger "
         "(default: 60)\n"
      << "  --retries <n>         Max webhook retry attempts (default: 3, 0 = "
         "no retry)\n"
      << "  --retry-delay <ms>    Delay between retries in ms (default: 1000)\n"
      << "  --no-ssl-verify       Disable SSL certificate verification (for "
         "self-signed certs)\n"
      << "  --max-line-length <n> Max characters per log line (default: 8192, "
         "0 = unlimited)\n"
      << "  --user <user>         Drop to this user after opening log "
         "(requires root)\n"
      ...
      << "  " << name
      << "                         # Start monitoring with auto-discovery\n"
...
static void parse_flags(Config &config, int argc, char *argv[], int start_idx,
                        bool handle_positional = false) {
